...
class leptop(Resource):
    ...
    def get(self):
        query = session.query(LeptopModel).order_by(LeptopModel.no)
        result_set = query.all()
        data = [{'no': row.no, 'brand': row.brand, 'ram': row.ram, 'cpu': row.cpu,
                'gpu': row.gpu, 'baterai': row.baterai, 'harga': row.harga, 'ukuran_layar': row.ukuran_layar}
                for row in result_set]
        return self.get_paginated_result('laptop/', data, request.args), 200


api.add_resource(leptop, '/laptop')
